...
// Almacenamiento simple en memoria + archivo JSON
const dataFile = path.join(__dirname, 'reportes.json');
let reportes = {};

// Cargar reportes existentes
if (fs.existsSync(dataFile)) {
  try { reportes = JSON.parse(fs.readFileSync(dataFile, 'utf8')); } catch(e) { reportes = {}; }
}

// Guardar reportes
const guardar = () => fs.writeFileSync(dataFile, JSON.stringify(reportes), 'utf8');
...
const limpiar = () => {
  ...
  Object.keys(reportes).forEach(id => {
    if (ahora - reportes[id].timestamp > dias7) delete reportes[id];
  });
  guardar();
};
limpiar();
setInterval(limpiar, 60 * 60 * 1000); // cada hora
        ...
        <div class="card-glass rounded-2xl p-6">
            ...
            <form id="formUpload" enctype="multipart/form-data">
                <div class="grid grid-cols-3 gap-3 mb-6">
                    <div class="drop-zone rounded-xl p-3 text-center cursor-pointer" id="dropMora" onclick="document.getElementById('fileMora').click()">
                        ...
                        <p class="text-xs text-gray-400" id="statusMora">Subir</p>
                    </div>
                    <div class="drop-zone rounded-xl p-3 text-center cursor-pointer" id="dropVigente" onclick="document.getElementById('fileVigente').click()">
                        ...
                        <p class="text-xs text-gray-400" id="statusVigente">Subir</p>
                    </div>
                    <div class="drop-zone rounded-xl p-3 text-center cursor-pointer" id="dropCobranza" onclick="document.getElementById('fileCobranza').click()">
                        ...
                        <p class="text-xs text-gray-400" id="statusCobranza">Subir</p>
                    </div>
                </div>
                <button type="submit" class="w-full py-3 bg-gradient-to-r from-cyan-600 to-blue-600 rounded-xl font-semibold hover:from-cyan-500 hover:to-blue-500">🚀 Procesar</button>
            </form>
            ...
            <div id="resultado" class="hidden mt-6 p-4 bg-emerald-900/30 rounded-xl border border-emerald-500/30">
                ...
                <div class="flex gap-2 mb-3">
                    ...
                    <button onclick="copiar()" class="px-3 py-2 bg-cyan-600 hover:bg-cyan-500 rounded-lg text-sm">📋</button>
                </div>
                <a id="linkIr" href="#" class="block text-center py-2 bg-violet-600 hover:bg-violet-500 rounded-lg text-sm font-medium">👁️ Ver Reporte</a>
            </div>
        </div>
    </div>
    <script>
        ['Mora','Vigente','Cobranza'].forEach(t=>{const d=document.getElementById('drop'+t);d.ondragover=e=>{e.preventDefault();d.classList.add('dragover')};d.ondragleave=()=>d.classList.remove('dragover');d.ondrop=e=>{e.preventDefault();d.classList.remove('dragover');if(e.dataTransfer.files[0]){const i=document.getElementById('file'+t),dt=new DataTransfer();dt.items.add(e.dataTransfer.files[0]);i.files=dt.files;handleFile(i,t)}}});
        function handleFile(i,t){if(i.files[0]){document.getElementById('drop'+t).classList.add('file-ok');document.getElementById('status'+t).textContent='✓'}}
        document.getElementById('formUpload').onsubmit=async e=>{e.preventDefault();document.getElementById('loading').classList.remove('hidden');document.getElementById('resultado').classList.add('hidden');try{const r=await fetch('/api/procesar',{method:'POST',body:new FormData(e.target)});const d=await r.json();if(d.success){document.getElementById('linkGenerado').value=location.origin+d.url;document.getElementById('linkIr').href=d.url;document.getElementById('resultado').classList.remove('hidden')}else alert('Error: '+d.error)}catch(err){alert('Error: '+err.message)}document.getElementById('loading').classList.add('hidden')};
        function copiar(){navigator.clipboard.writeText(document.getElementById('linkGenerado').value);alert('✓ Copiado!')}
    </script>
</body>
</html>`;
...
<body class="gradient-bg min-h-screen text-white">
    <div class="container mx-auto px-4 py-6 max-w-6xl">
        ...
        <div id="loading" class="text-center py-20"><div class="inline-block animate-spin rounded-full h-10 w-10 border-b-2 border-cyan-400"></div></div>
        <div id="contenido" class="hidden">
            <div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                ...
                <div class="bg-gradient-to-br from-amber-600 to-orange-700 rounded-xl p-4">
                    ...
                    <p class="text-amber-300 text-xs" id="kpiMoraD"></p>
                </div>
                <div class="bg-gradient-to-br from-violet-600 to-purple-800 rounded-xl p-4">
                    ...
                    <p class="text-violet-300 text-xs" id="kpiVigenteD"></p>
                </div>
            </div>
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-6">
                <div class="card-glass rounded-xl p-4"><h3 class="text-sm font-semibold mb-2">📍 Por Agencia</h3><div class="h-44"><canvas id="chart1"></canvas></div></div>
                <div class="card-glass rounded-xl p-4"><h3 class="text-sm font-semibold mb-2">📈 Distribución</h3><div class="h-44"><canvas id="chart2"></canvas></div></div>
            </div>
            <div class="card-glass rounded-xl p-4 mb-6">
                <h3 class="text-sm font-semibold mb-2">🏢 Por Agencia</h3>
                <div class="overflow-x-auto"><table class="w-full text-xs">
                    <thead><tr class="border-b border-gray-700"><th class="text-left p-2">Agencia</th><th class="text-center p-2">Mora</th><th class="text-center p-2">Vigente</th><th class="text-center p-2">Total</th><th class="text-center p-2">Cobrados</th><th class="text-center p-2">%</th><th class="text-right p-2">Monto</th></tr></thead>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div class="card-glass rounded-xl p-4">
                    <h3 class="text-sm font-semibold mb-2">⚠️ MORA con Pago (<span id="c1">0</span>)</h3>
                    <div class="overflow-y-auto max-h-40 text-xs"><table class="w-full"><tbody id="t1"></tbody></table></div>
                </div>
                <div class="card-glass rounded-xl p-4">
                    <h3 class="text-sm font-semibold mb-2">✅ VIGENTE con Pago (<span id="c2">0</span>)</h3>
                    <div class="overflow-y-auto max-h-40 text-xs"><table class="w-full"><tbody id="t2"></tbody></table></div>
                </div>
            </div>
    ...
    <script>
        ...
        fetch('/api/reporte/'+id).then(r=>r.json()).then(d=>{
            ...
            document.getElementById('kpiMoraD').textContent=r.mora.cobrados+'/'+r.mora.total+' $'+r.mora.monto.toLocaleString();
            document.getElementById('kpiVigente').textContent=r.vigente.porcentaje+'%';
            document.getElementById('kpiVigenteD').textContent=r.vigente.cobrados+'/'+r.vigente.total+' $'+r.vigente.monto.toLocaleString();
            document.getElementById('dl1').href='/api/dl/'+id+'/mora';
            document.getElementById('dl2').href='/api/dl/'+id+'/vigente';
            document.getElementById('dl3').href='/api/dl/'+id+'/reporte';
            let html='',tM=0,tV=0,tC=0,tCob=0,tMonto=0;
            r.porAgencia.forEach(a=>{
                const pc=a.porcentaje>=50?'text-emerald-400':a.porcentaje>=20?'text-yellow-400':'text-red-400';
                html+='<tr class="border-b border-gray-800"><td class="p-2">'+a.agencia+'</td><td class="p-2 text-center text-amber-400">'+a.mora+'</td><td class="p-2 text-center text-violet-400">'+a.vigente+'</td><td class="p-2 text-center">'+a.total+'</td><td class="p-2 text-center text-emerald-400">'+a.cobrados+'</td><td class="p-2 text-center '+pc+'">'+a.porcentaje+'%</td><td class="p-2 text-right text-emerald-400">$'+a.monto.toLocaleString()+'</td></tr>';
                tM+=a.mora;tV+=a.vigente;tC+=a.total;tCob+=a.cobrados;tMonto+=a.monto;
            });
            html+='<tr class="border-t-2 border-cyan-500 font-bold bg-slate-800/50"><td class="p-2">TOTAL</td><td class="p-2 text-center">'+tM+'</td><td class="p-2 text-center">'+tV+'</td><td class="p-2 text-center">'+tC+'</td><td class="p-2 text-center text-emerald-400">'+tCob+'</td><td class="p-2 text-center text-cyan-400">'+r.total.porcentaje+'%</td><td class="p-2 text-right text-emerald-400">$'+tMonto.toLocaleString()+'</td></tr>';
            document.getElementById('tabla').innerHTML=html;
            document.getElementById('c1').textContent=r.detalleMora.length;
            document.getElementById('c2').textContent=r.detalleVigente.length;
            document.getElementById('t1').innerHTML=r.detalleMora.map(c=>'<tr class="border-b border-gray-800"><td class="p-1">'+c.cliente+'</td><td class="p-1 text-amber-400">'+c.agencia+'</td><td class="p-1 text-right text-emerald-400">$'+c.cobranza.toLocaleString()+'</td></tr>').join('');
            document.getElementById('t2').innerHTML=r.detalleVigente.sort((a,b)=>b.cobranza-a.cobranza).map(c=>'<tr class="border-b border-gray-800"><td class="p-1">'+c.cliente+'</td><td class="p-1 text-violet-400">'+c.agencia+'</td><td class="p-1 text-right text-emerald-400">$'+c.cobranza.toLocaleString()+'</td></tr>').join('');
            new Chart(document.getElementById('chart1'),{type:'bar',data:{labels:r.porAgencia.map(a=>a.agencia),datasets:[{data:r.porAgencia.map(a=>a.cobrados),backgroundColor:'#10b981',borderRadius:4},{data:r.porAgencia.map(a=>a.total-a.cobrados),backgroundColor:'#475569',borderRadius:4}]},options:{responsive:true,maintainAspectRatio:false,scales:{x:{stacked:true,grid:{display:false},ticks:{color:'#94a3b8',font:{size:9}}},y:{stacked:true,grid:{color:'#334155'},ticks:{color:'#94a3b8'}}},plugins:{legend:{display:false}}}});
            new Chart(document.getElementById('chart2'),{type:'doughnut',data:{labels:['MORA sin pago','MORA cobrado','VIGENTE sin pago','VIGENTE cobrado'],datasets:[{data:[r.mora.total-r.mora.cobrados,r.mora.cobrados,r.vigente.total-r.vigente.cobrados,r.vigente.cobrados],backgroundColor:['#78350f','#f59e0b','#4c1d95','#8b5cf6'],borderWidth:0}]},options:{responsive:true,maintainAspectRatio:false,plugins:{legend:{position:'right',labels:{color:'#94a3b8',font:{size:9}}}}}});
...
app.get('/api/dl/:id/:tipo', (req, res) => {
  const r = reportes[req.params.id];
  if (!r) return res.status(404).send('No encontrado');
  const d = r.datos;
  let data, filename;
  if (req.params.tipo === 'mora') { data = d.moraData; filename = 'mora_actualizado.xlsx'; }
  else if (req.params.tipo === 'vigente') { data = d.vigenteData; filename = 'vigente_actualizado.xlsx'; }
  else {
...
function procesar(mora, vigente, cobranza) {
  ...
  const pagos = {};
  cobranza.forEach(r => {
    const n = norm(r.Nombre || r.NOMBRE || r.Cliente || r.CLIENTE);
    const m = limpiar(r.Cobrado || r.COBRADO || r.Monto || r.MONTO);
    if (n) pagos[n] = (pagos[n] || 0) + m;
  });
